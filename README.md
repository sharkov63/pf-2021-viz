...
## Тестирование
Исходный код сопровождён unit-тестами, покрывающими компоненты ввода и обработки данных. Кроме того, с помощью функций из класса ``src/test/kotlin/unitTests/AllSamples.kt`` можно получить картинки сразу всех диаграмм из ``samples`` всех вомзожных типов и сравнить их с эталонными (но для этого нужно сначала сгенерировать эталонные картинки другой функцией).

Также, с помощью класса ``src/test/kotlin/AllColors.kt`` можно сгенерировать картинки в директории ``samples/colors/``, представляющее некоторые доступные цвета диаграмм.